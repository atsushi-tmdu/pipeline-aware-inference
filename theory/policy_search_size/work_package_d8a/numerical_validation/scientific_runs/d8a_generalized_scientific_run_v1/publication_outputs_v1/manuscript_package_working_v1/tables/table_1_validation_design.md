...
| Exact evaluation identity | Eighty-five two-sided Bonferroni comparisons at familywise alpha=0.01 (z*=3.85098), targeting E(delta_hat)=(1-1/n)Delta_pi. | Provides a prespecified fatal implementation check. |
...
1. B denotes the reference-bank size, n the evaluation-sample size, alpha the TESS level, and tau the locked population target used only for the Monte Carlo stopping scale.
2. Normalized residuals use the prospectively specified simultaneous Monte Carlo uncertainty adjustment; raw-residual diagnostics are reported with the numerical results.
3. Detailed seed, transformation-audit, and role-specific replication specifications are provided in Supplementary Table S1.

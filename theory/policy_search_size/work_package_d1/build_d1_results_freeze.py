...
ARCHIVE = ARCHIVE_DIR / "d1_numerical_validation_v1_full_outputs.tar.gz"
ARCHIVE_SHA = ARCHIVE.with_suffix(ARCHIVE.suffix + ".sha256")
RESULTS_MANIFEST = FROZEN / "D1_RESULTS_FREEZE_MANIFEST.json"

LOCK_TAG = "tess-theory-work-package-d1-v1-lock-20260802"
LOCK_COMMIT = "01e8c0b"
RESULT_TAG = "tess-theory-work-package-d1-v1-results-review-20260802"
...
def main() -> None:
    ...
    if missing:
        raise SystemExit(
            "Required D1 full-run files are missing:\n- "
            + "\n- ".join(missing)
        )

    summary = json.loads(
        (SOURCE / "D1_VALIDATION_SUMMARY.json").read_text(encoding="utf-8")
    )
    expected = {
        "status": "REVIEW",
        "cell_count": 12,
        ...
        "failed_scientific_check_count": 14,
    }
    ...
    adjudication = HERE / "D1_POST_RUN_ADJUDICATION.md"
    if not adjudication.is_file():
        raise SystemExit("D1_POST_RUN_ADJUDICATION.md is missing")
    shutil.copy2(adjudication, FROZEN / adjudication.name)

    full_log = HERE / "D1_FULL_RUN_LOG.txt"
    review_output = HERE / "D1_REVIEW_OUTPUT.txt"
    ...
    archive_files.append(
        (adjudication, "D1_POST_RUN_ADJUDICATION.md")
    )
    if full_log.is_file():
    ...
    manifest = {
        ...
        "locked_status": "REVIEW",
        "scientific_interpretation": (
            "D1 first-order two-bank theory supported; "
            "basic bootstrap interval not uniformly validated"
        ),
        "lock_tag": LOCK_TAG,
    ...
    print("D1 results freeze created")
